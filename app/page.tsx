...
const tabs = [
  ["discover", "✦", "发现"], ["library", "▤", "论文库"], ["plan", "◷", "阅读计划"], ["atlas", "◎", "Atlas"],
] as const;
...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      {tab === "discover" && <Discover queue={queue} onAct={act} onReset={() => setQueue(SEED)} />}
      {tab === "library" && <Library states={states} onOpen={(p) => { setQueue([p, ...queue.filter(x => x.id !== p.id)]); setTab("discover"); }} />}
      {tab === "plan" && <Plan states={states} />}
      {tab === "atlas" && <Atlas />}
    </main>
    ...
    {showTopic && <TopicModal value={topic} onClose={() => setShowTopic(false)} onSave={(v) => { setTopic(v); setQueue(SEED); setShowTopic(false); setToast("推荐流已按新主题重排"); }} />}
    {toast && <div className="toast">✓ {toast}</div>}
  </div>;
}

function Discover({ queue, onAct, onReset }: { queue: Paper[]; onAct: (p: Paper, s: string) => void; onReset: () => void }) {
  const top = queue[0];
  return <section className="view discover-view">
    <div className="view-heading"><div><p className="eyebrow">TODAY'S DISCOVERY</p><h1>今天，发现点<span>新东西</span></h1><p>AI 从 3 个论文库筛选了 128 篇，只留下最值得你看的 6 篇。</p></div><div className="daily"><b>{Math.max(0, 6 - queue.length)}</b><span>/ 6 今日已处理</span></div></div>
    <div className="deck-area">
      <div className="deck">
        {queue.slice(0, 3).reverse().map((p, i, a) => <SwipeCard key={p.id} paper={p} top={i === a.length - 1} onAct={onAct} />)}
        {!top && <div className="done-card"><div>🎉</div><h2>今天的发现完成了</h2><p>你做出的选择会让明天的推荐更聪明。</p><button onClick={onReset}>再刷一轮演示</button></div>}
      </div>
...
function SwipeCard({ paper, top, onAct }: { paper: Paper; top: boolean; onAct: (p: Paper, s: string) => void }) {
  ...
  return <article className={`paper-card ${top ? "top" : "behind"}`} style={{ transform: top ? `translate(${drag.x}px,${drag.y}px) rotate(${drag.x/24}deg)` : undefined }} onPointerDown={(e) => { if(top){ origin.current={x:e.clientX,y:e.clientY}; e.currentTarget.setPointerCapture(e.pointerId); } }} onPointerMove={(e) => { if(origin.current) setDrag({x:e.clientX-origin.current.x,y:e.clientY-origin.current.y}); }} onPointerUp={end}>
    ...
    <div className="card-actions"><button aria-label="跳过" onClick={() => onAct(paper,"skipped")}>×</button><button aria-label="标为重点" onClick={() => onAct(paper,"priority")}>☆</button><button className="save-btn" onClick={() => onAct(paper,"saved")}><span>收藏论文</span> →</button><button aria-label="已读" onClick={() => onAct(paper,"read")}>✓</button></div>
  </article>;
}

function Library({ states, onOpen }: { states: Record<number,string>; onOpen:(p:Paper)=>void }) {
  const [q,setQ]=useState(""); const [filter,setFilter]=useState("all");
  const papers=useMemo(()=>SEED.filter(p=>(filter==="all"||states[p.id]===filter)&&(`${p.title}${p.zh}${p.tags}`.toLowerCase().includes(q.toLowerCase()))),[q,filter,states]);
  return <section className="view list-view"><div className="page-title"><div><p className="eyebrow">YOUR LIBRARY</p><h1>论文库</h1><p>每一次滑动都变成可找回、可行动的研究资产。</p></div><button className="primary">导出 BibTeX</button></div><div className="toolbar"><label>⌕<input value={q} onChange={e=>setQ(e.target.value)} placeholder="搜索标题、作者或标签"/></label><div>{[["all","全部"],["saved","感兴趣"],["priority","重点"],["read","已读"]].map(([id,l])=><button className={filter===id?"active":""} onClick={()=>setFilter(id)} key={id}>{l}</button>)}</div></div><div className="paper-list">{papers.map(p=><article key={p.id}><div className="mini-cover">{p.venue}<small>{p.year}</small></div><div className="paper-info"><div className="tags">{p.tags.slice(0,2).map(t=><span key={t}>#{t}</span>)}</div><h3>{p.zh}</h3><p>{p.title}</p><small>{p.authors} · {p.citations.toLocaleString()} 次引用</small></div><div className="row-score"><b>{p.score}%</b><span>相关</span></div><button onClick={()=>onOpen(p)}>打开卡片 →</button></article>)}</div></section>;
}

function Plan({states}:{states:Record<number,string>}) { const chosen=SEED.filter(p=>states[p.id]==="priority"||states[p.id]==="saved").slice(0,3); const list=chosen.length?chosen:SEED.slice(0,3); return <section className="view list-view"><div className="page-title"><div><p className="eyebrow">READING PLAN</p><h1>把收藏变成读完</h1><p>AI 按优先级、论文类型和你的空闲时间安排阅读。</p></div><button className="primary">让 AI 重新规划</button></div><div className="week"><div className="week-summary"><span>本周 · 7月27日—8月2日</span><b>3.2 小时</b><em>计划阅读</em><div><i style={{width:"62%"}}></i></div><small>2 / 5 篇已完成</small></div>{["今天 · 周三","明天 · 周四","周五"].map((d,i)=><article key={d}><div className="day"><b>{d.split(" · ")[0]}</b><span>{d.split(" · ")[1]}</span></div><div className="plan-paper"><span className={i===0?"priority-dot":""}></span><div><small>{list[i].verdict} · {list[i].minutes} 分钟</small><h3>{list[i].zh}</h3><p>{list[i].authors}</p></div><button>开始读 →</button></div></article>)}</div></section> }
